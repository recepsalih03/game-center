...
import { Box, Container, CssBaseline, ThemeProvider, createTheme, Tabs, Tab, Grid, Button } from "@mui/material"
...
const TabPanel:React.FC<{ index:number; value:number; children:React.ReactNode }> =
  ({ index,value,children }) => (index===value ? <Box pt={2}>{children}</Box> : null)
    ...
    <ThemeProvider theme={lightTheme}>
      ...
      <Container maxWidth="lg" sx={{ mt:4, mb:4 }}>
        <Tabs value={tab} onChange={(_,v)=>setTab(v)} textColor="primary" indicatorColor="primary" sx={{ mb:2 }}>
          {["Overview","Lobbies","History","How to Play","Settings"].map((lbl,i)=><Tab key={i} label={lbl}/>)}
        </Tabs>
